# Useful-Resources
These are a whole bunch of websites that I've found useful as I learn systems programming.

# ECS
## Anything by Sander Mertens
- https://ajmmertens.medium.com/
- https://github.com/SanderMertens/ecs-faq
- https://skypjack.github.io/
## Basics
- https://www.david-colson.com/2020/02/09/making-a-simple-ecs.html
- https://ajmmertens.medium.com/doing-a-lot-with-a-little-ecs-identifiers-25a72bd2647
- https://devlog.hexops.com/2022/lets-build-ecs-part-1/
## Sparse Sets and Advanced Data Storage
- [cient - Optimizing My ECS Game Engine to Simulate 100,000 Entities | Sparse sets](https://youtu.be/yyZMoE1FAJ0)
## Other
- [Bob Nystrom - Is There More to Game Architecture than ECS?](https://youtu.be/JxI3Eu5DPwE)
- [UnitOfTime - Building a fast ECS on top of a slow ECS](https://youtu.be/71RSWVyOMEY)
## General Things to Research
- Job Systems
  - ECS is _highly_ multi-threadable
- Scheduling Algorithms
- Archetype-based ECS'
# Scripting Language Interpreters
[The holy bible - Crafting Interpreters by Bob Nystrom](https://craftinginterpreters.com/)
